    ...
    <div class="grid grid-cols-1 gap-6">
        <div id="whitelist-settings">
            ...
            <form method="post" action="settings#whitelist-settings">
                ...
                <div class="mb-6">
                    <label for="whitelist-roles" class="block text-sm font-medium text-gray-400 mb-2">Rôles (séparés par des virgules) :</label>
                    <input type="text" class="form-input mt-1 block w-full rounded-lg border-gray-600 bg-gray-700 text-gray-200 p-2 focus:ring-indigo-500 focus:border-indigo-500" id="whitelist-roles" name="whitelist_roles" value="<?php
                        $sql = "SELECT role FROM whitelist_roles"; 
                        $stmt = $pdo->query($sql);

                        $roleNames = array();

                        while ($whitelist_role = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            $roleNames[] = $whitelist_role["role"];
                        }
